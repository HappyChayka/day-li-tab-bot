...
class ThrottlingMiddleware(BaseMiddleware):
    def __init__(self, redis: redis.asyncio.client.Redis, limit=2, key_prefix='antiflood_'):
...
class ThrottleManager:
    ...
    async def throttle(self, key: str, rate: float, user_id: int, chat_id: int):
        ...
        await self.redis.hset(bucket_name, data)
...
class Throttled(Exception):
    ...
    def __str__(self):
        return f"Ограничение вызовов превышено! (Ограничение: {self.rate} с, " \
               f"превышено: {self.exceeded_count}, " \
               f"время: {round(self.delta, 3)} с)"
